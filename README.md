...
对标 pi 的自扩展能力，规划中的**插件运行时**利用 Racket 的解释/拓展能力：
`dynamic-require` 载入受信插件、`racket/sandbox` 在受限求值器里安全运行**不可信**插件
（限内存/时间/文件/网络 + custodian 回收——pi 缺失的能力），扩展工具/命令/钩子/供应商。
设计与分阶段方案见 [design-plugins.md](design-plugins.md)；最小原型见 `src/plugin.rkt`、
`plugins/`、`tests/plugin-test.rkt`（受信载入、沙箱执行、失控插件被关停而宿主存活）。
