...
`/help` `/quit` `/clear` `/usage` `/compact` `/history` `/tail [n]` `/resume` `/skills` `/prompt [name]` `/provider [name]` `/reasoning [level]` `/model <id>`
...
│   ├── provider-anthropic.rkt 原生 Messages 线路  providers.rkt 内置供应商档案
