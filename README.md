...
- **工作动画**：LLM 执行任务的**整轮**期间，底边隔离条播放进度动画——能力足够的 tty
  （truecolor/256 色）播 **256 色块进度条**（青色渐变块左右横扫），基础 tty 用**隔离条扫光**
  （粗亮线在暗细线上掠过，纯 bold/dim 无色，处处可用）；两形式依 tty 自动选择，
  `PI_PROGRESS=bar|sweep` 可强制。动画**速率按实时 token/s 估计分 4 档**（越快扫得越快），
  隔离条右端并显示 `~N tok/s` 读数，指征运算性能。等待首 token / 工具间隙时状态行另显转轮 + `thinking…`。
