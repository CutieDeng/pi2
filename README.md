...
→ {"type":"state"} / {"type":"history"}      // 查询（state 含 reasoning/auto/fallback/cost_usd）
...
`/help` `/quit` `/clear` `/usage` `/cost` `/compact` `/history` `/tail [n]` `/resume` `/skills` `/prompt [name]` `/provider [name|add|key]` `/reasoning [level]` `/auto [on|off]` `/fallback [targets|clear]` `/model <id>`
