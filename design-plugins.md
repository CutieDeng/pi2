...
- **M4 多供应商 + ctx.ui** ✅（键位待接）：**`register-provider!`**——host 存 name→工厂(config→provider)，
...
**多供应商测试**：插件 `register-provider!` 注册 `echollm`（`host-provider-names` 含之），经 `run-turn!`
用该供应商产出回复；`ctx.select`/`confirm` 经 host 注入正确路由。pty：`--provider echollm` 下模型回复
由**插件供应商**产出（`echo-llm reply: …` 上屏）——证明多供应商端到端。

后续按 §7 的 M4 键位 / M5（`#lang pi/plugin` DSL、技能/提示词）推进。
