...
- **底部固定「输入框」+ 上方滚动输出**：屏幕底部动态区自顶向下 =
  `[可选 pending 半行] + [分隔线] + [可选命令预览] + [提示行(光标)]`，异步输出一律滚动到其上方。
  **分隔线**（整宽暗色 `─`）把上文交互内容与输入框清晰隔开。输入为空且正在跑 turn 时隐藏
  整个输入框，输出自然贴底（无多余空提示符）；用户一旦开始键入，输入框即刻钉底。
- **单锁串行化一切终端写**：reader 线程的按键回显、main 线程的流式输出、权限询问都走
  同一把 `semaphore`。`console-emit!` 每次落屏都：擦掉动态区 → 提交完整输出行为永久滚动历史
  （`\n` 规整为 `\r\n`，并入滚动缓存）→ 底部重绘输入框。故用户已键入内容始终正确回显，
  不与异步消息冲突。多行输出按 `\n` 切分逐行提交，未换行尾入 `pending` 于框上方滚动显示。
- **重绘的行数记账**：`rows` 记录上次动态区占用行数（含 pending/分隔线/预览/提示行），重绘先
  `\e[{n}A` 上移再 `\e[J` 清屏；用**相对**光标移动，滚动安全，多行预览面板增删亦正确。
- **Ctrl-C 阶梯语义**（`handle-cancel!`）：raw 模式下 Ctrl-C 不产生 SIGINT，由 reader 决策——
  **有草稿** → 清空输入框（不打断）；**草稿为空且运行中** → 回显 `^C` 并 `break-thread` 主线程，
...
- **可离线测试**：`console-handle-key!` / `console-emit!` 均为同步纯写协调，配脚本化终端即可
  断言「键入 abc 时输出 line1 不撕裂 abc」「\n 规整为 \r\n」「Ctrl-C 四态」「分隔线/命令预览
  渲染」「缓存环形淘汰」等（`tests/tui-console-test.rkt`），线程与取消策略留在 repl，不入测试路径。
