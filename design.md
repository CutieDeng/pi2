...
(provide
  (contract-out
    ;; 决策：allow / deny / (cons 'deny reason)；ask 时经 asker 回调询问用户
    [permission-check
      (-> permission-policy?
          tool? jsexpr?                 ; 哪个工具、什么参数
          ;; asker：阻塞式询问，返回 'yes | 'always | 'no | (cons 'no reason)
          (-> string? (or/c 'yes 'always 'no (cons/c 'no string?)))
          (or/c 'allow 'deny (cons/c 'deny string?))
      ) ; ->
    ] ; permission-check
...
**审批 UI 与拒绝理由**：需 `ask` 时，交互式 asker（`repl.rkt` `interactive-approve`）在屏幕底部弹
**内联小选框**（`console-choose!`：Yes 本次 / Yes 不再问 / No / No+理由；↑/↓/Enter，Esc=拒绝），
取代 y/n/a 回显输入——底部框区改画标题+选项、**上方对话仍可见**（区别于全屏 `console-pick!`）；
导航复用 `pick-step`。选「No+理由」再经 `console-ask!` 收一行理由。asker 把理由以 `(cons 'no reason)` 上交，
`permission-check` 转成 `(cons 'deny reason)`，`loop.rkt` 把它写进**回填给模型的 tool-result**
（「用户拒绝，理由 X，勿重试、据此调整」），从而让模型在类似调用下表现更好。管道/非交互
回退 `tty-asker` 的纯 y/n/a 文本。
...
- **自有滚动视口**：`view` = 上滚行数（0=跟随底部）。`keys.rkt` 解析 **SGR 鼠标滚轮**
  ...
  提交新输入即 `view←0` 跳回底部。
- **工作动画**：三路。① 整轮的**底边隔离条进度动画**——repl 在开跑前 `console-set-working! #t`、
  turn 末 `#f`；`working` 为真时 animator 每 120ms 推进相位并重绘，`separator-line` 依 tty 能力渲染：
  `'bar`（**默认**）画青色渐变**细线**（`━` 而非实心块，占位小）乒乓横扫，`'sweep`（`PI_PROGRESS=sweep`）
  画粗亮线单向**扫光**（纯 bold/dim 无色，处处可用）；`detect-progress-style` 默认 `'bar`，`PI_PROGRESS=sweep`
  取 `'sweep`。`console-set-working! #t` 时**把相位 `spin` 归零**，故每轮动画从起点重来、不续上一轮。
  **速率 4 档 + 平滑**：渲染器把每段 delta 字符数经 `console-tick-tokens!` 上报，animator `sample-rate!`
  每 `SAMPLE-MS`(400ms) 结算一次窗口平均并经低 α(0.25) EMA 低通（长窗 + 低通抑抖），`rate->step` 映射为
  相位步长 1/2/3/4（越快扫得越快）；`progress-label` 把速率 `quantize-rate` 量化到 5 后显 `~N tok/s`。② **转轮标签**（`status`，
  首 token/工具间隙）显 `⠋ thinking…`。③ **OSC 9;4 终端原生进度条**：`console-set-working!` 顺带发
  `OSC 9;4;3`（不定进度）/`9;4;0`（清除），由 emulator（Ghostty/WezTerm/Windows Terminal/iTerm2 3.5+）
  在其 chrome 画原生进度条——非 tty 内动画，未支持者忽略未知 OSC；`detect-osc-progress` 依 `TERM_PROGRAM`
  /`WT_SESSION` 自动开、`PI_OSC_PROGRESS` 强制，`console-stop!` 兜底清除。animator 仅在 `working`/`status`
  为真且非选框/选择器交互时推进（相位 mod 100000）；每 ~1s 复查尺寸做 resize（`clamp-dim` 夹紧）。
...
- **'/' 命令实时预览 + Tab 补全**：输入以 `/` 起头时底部实时渲染匹配命令（`#:hint`）；`Tab`
  经 `#:complete` 回调补全命令名——唯一匹配补全为 `/name `，多匹配补到最长公共前缀，
  已入参数区/无进展则不动。命令知识（`COMMANDS` 表）留在 repl，console 只负责截键与替换行。
- **询问改道**（`console-ask!`）同前。
...
console 因此有两种基于 `pick-step` 的选择模式：`console-pick!`（**全屏**列表，`/resume` 用）与
`console-choose!`（**贴底内联小选框**，权限审批用——底部框区改画标题+选项、上方对话仍可见，
见 §4.7）。二者都由 reader 线程驱动、经 channel 回投，与 `console-ask!` 同一套并发骨架。
