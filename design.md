...
**审批 UI 与拒绝理由**：需 `ask` 时，交互式 asker（`repl.rkt` `interactive-approve`）弹**选框**
（复用 `console-pick!`：Yes 本次 / Yes 不再问 / No / No+理由；Esc=拒绝），取代 y/n/a 回显输入；
选「No+理由」再经 `console-ask!` 收一行理由。asker 把理由以 `(cons 'no reason)` 上交，
