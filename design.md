...
- **工作动画**：三路。① 整轮的**底边隔离条进度动画**——repl 在开跑前 `console-set-working! #t`、
  ...
  `'bar`（truecolor/256 色）画青色渐变块**进度条**乒乓横扫，`'sweep`（基础 tty）画粗亮线在暗线上
  **扫光**（纯 bold/dim 无色，处处可用）；形式由 `detect-progress-style` 依 `COLORTERM`/`TERM` 选，
  `PI_PROGRESS=bar|sweep` 强制。**速率 4 档**：渲染器把每段 delta 字符数经 `console-tick-tokens!` 上报，
  animator `sample-rate!` 换算成 token/s（÷2.5 粗估 + EMA），`rate->step` 映射为相位步长 1/2/3/4
  （越快扫得越快）；隔离条右端 `progress-label` 显 `~N tok/s`，指征运算性能。② **转轮标签**（`status`，
