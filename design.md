...
- **自有滚动视口**：`view` = 上滚行数（0=跟随底部）。`keys.rkt` 解析 **SGR 鼠标滚轮**
  ...
  提交新输入即 `view←0` 跳回底部。
- **工作动画**：两级。① 整轮的**底边隔离条进度动画**——repl 在开跑前 `console-set-working! #t`、
  ...
  **扫光**（纯 bold/dim 无色，处处可用）；形式由 `detect-progress-style` 依 `COLORTERM`/`TERM` 选，
  `PI_PROGRESS=bar|sweep` 强制。② 等待首 token / 工具间隙的**转轮标签**：`status` 置标签时状态行显
  `⠋ thinking…`；repl 开跑前 `statusf #t`，`console-emit!` 一有输出即清零，`evt:tool-end` 再置以覆盖
  空档，turn 末清零。animator 仅在 `working`/`status` 为真且非选框/选择器交互时推进（相位 mod 100000）。
  同一 animator 每 ~1s 复查终端尺寸，变化即重排（resize）；尺寸经 `clamp-dim` 夹紧。
