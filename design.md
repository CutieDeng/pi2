...
(provide
  (contract-out
    [session-open! (-> path-string? session?)]           ; 打开或新建
    [session-append! (-> session? rec? void?)]           ; 追加一条记录
    [session-replay (-> path-string? agent-state?)]      ; 流式重放重建状态
    [session-list (-> path-string? (listof session-meta?))]
  ) ; contract-out
) ; provide
```
...
- 只追加、不改写 → 崩溃安全：`read` 遇残缺尾 datum 抛 `exn:fail:read`，
  重放时捕获并丢弃即可，无需锁。
- `session-list` 只需读每个文件的首个 datum（`meta` 记录）即可列出元信息 ——
  datum 流「读一个就停」的能力让列表操作 O(1) 每文件。
...
- **底部固定「输入框」+ 上方滚动输出**：屏幕底部动态区自顶向下 =
  `[可选 pending 半行] + [分隔线] + [可选命令预览] + [提示行/多行输入(光标)]`，异步输出一律
  滚动到其上方。**分隔线**（整宽暗色 `─`）把上文交互内容与输入框清晰隔开。**输入框恒显**：
  即便 agent 处理消息、输入为空时也在末端展示框与光标，故 LLM 回显过程中光标只在框内、
  不驻留输出体。
- **帧式写入**：每个可见操作把整帧组装成一个字符串，用 `\e[?25l …帧… \e[?25h` 括起单次写入
  （`frame!`）——写时藏光标、写毕在框内复现，输出体上无光标游走，整帧原子。
- **单锁串行化一切终端写**：reader 线程的按键回显、main 线程的流式输出、权限询问都走
  同一把 `semaphore`。`console-emit!` 每次落屏都：擦动态区 → 提交完整输出行为永久滚动历史
  （`\n` 规整为 `\r\n`，并入滚动缓存）→ 底部重绘输入框。多行输出按 `\n` 切分逐行提交，
  未换行尾入 `pending` 于框上方滚动显示。
- **多行输入**：`Shift/Alt+Enter` 在编辑器插入 `\n`（普通 Enter 提交）；`ledit-render` 多行感知，
  逐行输出并把光标上移到其所在行、右移到目标列；提交时按等宽缩进回显各续行。**空输入回车
  不派发**、仅换行（框下移一行）。`keys.rkt` 识别 Shift+Enter 的 CSI-u（`\e[13;2u`，kitty 键协议）
  与 modifyOtherKeys（`\e[27;2;13~`）两种上报，及 Alt+Enter（`\e\r`）可移植回退；`console-start!`
  开启 `modifyOtherKeys=1` 让 xterm 系终端上报 Shift+Enter（不支持者忽略，退回 Alt+Enter）。
- **重绘的行数记账**：`curup` 记录「上次绘制后光标距动态区顶部的行数」（含 pending/分隔线/
  预览/多行输入中光标所在行），重绘先 `\e[{curup}A` 上移到顶再 `\e[J` 清屏；用**相对**光标移动，
  滚动安全，且正确处理多行输入光标不在末行、命令预览增删等情形。
- **Ctrl-C 阶梯语义**（`handle-cancel!`，**不回显 `^C`**）：raw 模式下 Ctrl-C 不产生 SIGINT，由
  reader 决策——**有草稿** → 清空输入框（不打断）；**草稿为空且运行中** → `break-thread` 主线程，
  主线程 `parameterize-break` 内的 `run-turn!` 收 `exn:break` 取消当轮并 `provider-cancel!`，
  中断提示 `⎯ interrupted ⎯` 由 repl 先 `emit "\n"` 收尾再单起一行给出，**作为独立元信息块、
  不与输出回显同块**；**草稿为空且空闲** → 无动作。
- **'/' 命令实时预览**：输入以 `/` 起头时，`#:hint` 回调（由 repl 按 `COMMANDS` 表提供）返回
  匹配命令的预览行，渲染在提示行**上方**（故 `ledit-render` 仍是最后一步、光标定位不受影响），
  边打边收窄。命令知识留在 repl，console 只负责渲染，解耦干净。
...
- **可离线测试**：`console-handle-key!` / `console-emit!` 均为同步纯写协调，配脚本化终端即可
  断言「输出不撕裂已键入内容」「\n 规整为 \r\n」「Ctrl-C 四态、无 ^C 回显」「空回车不派发」
  「Shift/Alt+Enter 多行提交」「帧式藏/显光标」「分隔线/命令预览/缓存环形淘汰」等
  （`tests/tui-console-test.rkt`），线程与取消策略留在 repl，不入测试路径。

### 11.4 缓存与滚动（超长会话）

- **原生 scrollback 优先**：输出全部留在**主屏**（不进 alternate screen、不抢占鼠标上报），
  故终端自带的 scrollback 生效，鼠标滚轮/触控板上翻历史开箱即用——刻意不 grab 鼠标事件，
  否则会破坏原生滚动却又得自建视口，得不偿失。
- **内存滚动缓存**：`console` 内嵌定长**环形缓冲**（`ring`，默认 4000 行），每提交一行输出即
  `ring-push!`（存去 ANSI 的纯文本），O(1) 追加、自动淘汰最旧，界定超长会话的内存占用。
  `console-tail-lines` / `/tail [n]` 据此做**局部信息提取**（取最后 n 行）。
...
真实 tty 经 pty 实测：逐键重绘、CJK 双宽光标对齐、分隔线与 `/` 命令实时预览、提交行回显、
流式答案落屏、流式中 Ctrl-C 打断并回显后 REPL 存活续答、Ctrl-D 干净退出均正确。
